...
import { RESTInfo } from "../types/RestaurantMenuTypes/RESTInfoType";
import { RESTOffer } from "../types/RestaurantMenuTypes/restOfferTypes";
import { RESTMenus } from "../types/RestaurantMenuTypes/restMenusType";

const useRestaurant = (resId: string | undefined) => {
  const [restaurantMenu, setRestaurantMenu] = useState<{
    restInfo: RESTInfo | undefined;
    restOffer: RESTOffer | undefined;
    restMenus: RESTMenus | undefined;
  } | null>(null);
  ...
  const getRestaurantMenu = async () => {
    try {
      ...
      const restOffer: RESTOffer = json?.data?.cards?.find((res) =>
        res?.card?.card?.gridElements?.infoWithStyle["@type"]?.includes(
          "food.v2.OfferInfoWithStyle"
        )
      );

      const restMenus: RESTMenus = json?.data?.cards?.find((res) =>
